...
(function () {
    ...
    QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
        ...
        for (const element of rsBlocks) {
            totalDataCount += element.dataCount;
        }
    ...
    QRCodeModel.createBytes = function (buffer, rsBlocks) {
        ...
        for (const element of rsBlocks) {
            totalCodeCount += element.totalCount;
        }
    ...
    let Drawing = (function () {
        ...
        Drawing.prototype.draw = function (oQRCode) {
            ...
            let bgWidth = _htOption.width;
            let bgHeight = _htOption.height;
            let top = _htOption.top;
            let left = _htOption.left;
            let materials = _htOption.materials;
            ...
            for (let row = 0; row < nCount; row++) {
                for (let col = 0; col < nCount; col++) {
                    ...
                    let nLeft = col * nWidth + left;
                    let nTop = row * nHeight + top;
    ...
    QRCode.prototype.loadMaterial = function (sText) {
        ...
        for (const element of this.materials) {
            let prop = element;
            let src = this._htOption.materials[prop];
            ...
            if (!src) {
                ...
                for (let j = 0; j < src.length; j++) {
                    ...
                    let childSrc = this._htOption.materials[prop][j];
